...
package com.android.weici.common.widget.subscaleview;
...
import com.android.weici.common.R.styleable;
import com.android.weici.common.widget.subscaleview.decoder.CompatDecoderFactory;
import com.android.weici.common.widget.subscaleview.decoder.DecoderFactory;
import com.android.weici.common.widget.subscaleview.decoder.ImageDecoder;
import com.android.weici.common.widget.subscaleview.decoder.ImageRegionDecoder;
import com.android.weici.common.widget.subscaleview.decoder.SkiaImageDecoder;
import com.android.weici.common.widget.subscaleview.decoder.SkiaImageRegionDecoder;
...
public class SubsamplingScaleImageView extends View {
	...
	private static final List<Integer> VALID_SCALE_TYPES = Arrays
			.asList(SCALE_TYPE_CENTER_CROP, SCALE_TYPE_CENTER_INSIDE,
					SCALE_TYPE_CUSTOM);
	...
	public SubsamplingScaleImageView(Context context, AttributeSet attr) {
		...
		if (attr != null) {
			...
			if (typedAttr
					.hasValue(styleable.SubsamplingScaleImageView_quickScaleEnabled)) {
				setQuickScaleEnabled(typedAttr.getBoolean(
						styleable.SubsamplingScaleImageView_quickScaleEnabled,
						true));
			}
			if (typedAttr
					.hasValue(styleable.SubsamplingScaleImageView_tileBackgroundColor)) {
	 ...
	 * screen orientation change; it avoids any redundant loading of tiles in
	 ...
	 * {@link ImageSource#dimensions(int, int)} on the imageSource object. The
	 * preview source will be ignored if you don't provide dimensions, and if
	 * you provide a bitmap for the full size image.
	 * 
	 * @param imageSource
	 *            Image source. Dimensions must be declared.
	 * @param previewSource
	 *            Optional source for a preview image to be displayed and allow
	...
	public final void setImage(ImageSource imageSource,
		...
		if (imageSource.getBitmap() != null && imageSource.getSRegion() != null) {
			onImageLoaded(Bitmap.createBitmap(imageSource.getBitmap(),
					...
					imageSource.getSRegion().top, imageSource.getSRegion()
							.width(), imageSource.getSRegion().height()),
					ORIENTATION_0);
				...
				new GestureDetector.SimpleOnGestureListener() {
					...
					public boolean onDoubleTap(MotionEvent e) {
						if (zoomEnabled && readySent && vTranslate != null) {
							...
							// where the next fling is ignored, so here we
							// replace it with a new one.
							setGestureDetector(context);
							if (quickScaleEnabled) {
								// Store quick scale params. This will become
								// either a double tap zoom or a
								// quick scale depending on whether the user
								// swipes.
								vCenterStart = new PointF(e.getX(), e.getY());
								vTranslateStart = new PointF(vTranslate.x,
										vTranslate.y);
								scaleStart = scale;
								isQuickScaling = true;
								isZooming = true;
								...
								quickScaleLastPoint = new PointF(
										quickScaleCenter.x, quickScaleCenter.y);
								quickScaleMoved = false;
								// We need to get events in onTouchEvent after
								// this.
								return false;
							} else {
								// Start double tap zoom animation.
								doubleTapZoom(
										viewToSourceCoord(new PointF(e.getX(),
												e.getY())), new PointF(
												e.getX(), e.getY()));
								return true;
							}
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (maxTouchCount > 0) {
				if (touchCount >= 2) {
					...
					float vDistEnd = distance(event.getX(0), event.getX(1),
							event.getY(0), event.getY(1));
					float vCenterEndX = (event.getX(0) + event.getX(1)) / 2;
					float vCenterEndY = (event.getY(0) + event.getY(1)) / 2;

					if (zoomEnabled
							&& (distance(vCenterStart.x, vCenterEndX,
									vCenterStart.y, vCenterEndY) > 5
					...
					// One finger zoom
					// Stole Google's Magical Formula鈩� to make sure it feels
					// the exact same
					...
					float spanDiff = (Math
							.abs(1 - (dist / quickScaleLastDistance)) * 0.5F);
					...
					if (spanDiff > 0.03f || quickScaleMoved) {
						...
						if (quickScaleLastDistance > 0) {
							multiplier = isUpwards ? (1 + spanDiff)
									: (1 - spanDiff);
						}

						scale = Math.max(minScale(),
	...
	protected void onDraw(Canvas canvas) {
		...
		// When using tiles, on first render with no tile map ready, initialise
		// it and kick off async base image loading.
		if (tileMap == null && decoder != null) {
			initialiseBaseLayer(getMaxBitmapDimensions(canvas));
		}

		// If image has been loaded or supplied as a bitmap, onDraw may be the
			...
			if (debug) {
				...
				canvas.drawText(
						"Translate: " + String.format("%.2f", vTranslate.x)
								+ ":" + String.format("%.2f", vTranslate.y), 5,
						35, debugPaint);
				PointF center = getCenter();
				canvas.drawText(
						"Source center: " + String.format("%.2f", center.x)
								+ ":" + String.format("%.2f", center.y), 5, 55,
						debugPaint);
				...
				if (tile.sampleSize == sampleSize) {
					if (tileVisible(tile)) {
						...
						if (!tile.loading && tile.bitmap == null && load) {
							TileLoadTask task = new TileLoadTask(this, decoder,
									tile);
							task.execute();
						}
	...
	private void fitToBounds(boolean center, ScaleAndTranslate sat) {
		...
		float xPaddingRatio = getPaddingLeft() > 0 || getPaddingRight() > 0 ? getPaddingLeft()
				/ (float) (getPaddingLeft() + getPaddingRight())
				: 0.5f;
		float yPaddingRatio = getPaddingTop() > 0 || getPaddingBottom() > 0 ? getPaddingTop()
	...
	private static class TilesInitTask extends AsyncTask<Void, Void, int[]> {
		...
		protected int[] doInBackground(Void... params) {
			try {
				...
				DecoderFactory<? extends ImageRegionDecoder> decoderFactory = decoderFactoryRef
						.get();
			...
			if (view != null) {
				if (decoder != null && xyo != null && xyo.length == 3) {
					view.onTilesInited(decoder, xyo[0], xyo[1], xyo[2]);
						...
						&& view.onImageEventListener != null) {
	...
	private synchronized void onTilesInited(ImageRegionDecoder decoder,
			int sWidth, int sHeight, int sOrientation) {
			...
			try {
				...
				if (decoder != null && tile != null && view != null
					...
					synchronized (view.decoderLock) {
						...
						if (view.sRegion != null) {
							tile.fileSRect.offset(view.sRegion.left,
									view.sRegion.top);
						}
						return decoder.decodeRegion(tile.fileSRect,
								tile.sampleSize);
					}
				} else if (tile != null) {
					tile.loading = false;
				...
				Logger.e(TAG, "Failed to decode tile", e);
				this.exception = e;
			}
			return null;
		}
	...
	private static class BitmapLoadTask extends AsyncTask<Void, Void, Integer> {
		...
		protected Integer doInBackground(Void... params) {
			try {
				String sourceUri = source.toString();
				Context context = contextRef.get();
				DecoderFactory<? extends ImageDecoder> decoderFactory = decoderFactoryRef
						.get();
	...
	private synchronized void onPreviewLoaded(Bitmap previewBitmap) {
		...
		if (pRegion != null) {
			bitmap = Bitmap.createBitmap(previewBitmap, pRegion.left,
	...
	private synchronized void onImageLoaded(Bitmap bitmap, int sOrientation) {
		...
		if (this.sWidth > 0
				&& this.sHeight > 0
				&& (this.sWidth != bitmap.getWidth() || this.sHeight != bitmap
						.getHeight())) {
			reset(false);
		}
		if (this.bitmap != null) {
		...
		if (sourceUri.startsWith(ImageSource.FILE_SCHEME)
			...
			try {
				...
				if (orientationAttr == ExifInterface.ORIENTATION_NORMAL
					...
					Logger.w(TAG, "Unsupported EXIF orientation: "
							+ orientationAttr);
				}
			} catch (Exception e) {
				Logger.w(TAG, "Could not get EXIF orientation of image");
			}
		}
	...
	private static class Tile {
		...
		// Volatile fields instantiated once then updated before use to reduce
		// GC.
		private Rect vRect;
	...
	private void fileSRect(Rect sRect, Rect target) {
		if (getRequiredRotation() == 0) {
			...
			target.set(sRect.top, sHeight - sRect.right, sRect.bottom, sHeight
					- sRect.left);
	 ...
	 * any fields that use significant memory. After you have called this
	 * method, the view can be re-used by setting a new image. Settings are
	 * remembered but state (scale and center) is forgotten. You can restore
	 ...
	 * Accepts the desired scale as an argument, so this is independent of
	 * current translate and scale. The result is fitted to bounds, putting the
	 * image point as near to the screen center as permitted.
	 */
	 ...
	 * custom decoder when using layout XML to set an asset name. Your class
	 ...
	 *            The {@link DecoderFactory} implementation that produces
	...
	public final void setBitmapDecoderClass(
			Class<? extends ImageDecoder> bitmapDecoderClass) {
	 ...
	 * scale. You may wish to set this according to screen density. Consider
	 * using {@link #setMaximumDpi(int)}, which is density aware.
	 */
	 ...
	 * ; it allows you to express the minimum allowed scale in terms of the
	 ...
	 * usually be enough. This should be called before setting the image source,
	 * because it affects which tiles get loaded. When using an untiled source
	 * image this method has no effect.
	 * 
	 ...
	 * be the preview that will be displayed and the full size image may still
	 * be loading. If no preview was provided, this is called once the base
	 ...
	 * display an image on the next draw. This is triggered at the same time as
	 * {@link OnImageEventListener#onReady()} but allows a subclass to receive
	 * this event without using a listener.
	 */
	 ...
	 * have been loaded. Before this event the view is blank unless a preview
	 * was provided.
	 */
	 ...
	 *         {@link AnimationBuilder#start()} to
	 *         start the anim.
	 */
	...
	public final class AnimationBuilder {
		...
		public void start() {
			...
			PointF targetSCenter = panLimited ? limitedSCenter(
					this.targetSCenter.x, this.targetSCenter.y, targetScale,
			...
			if (vFocus != null) {
				...
				anim.vFocusEnd = new PointF(vFocus.x
						+ (satEnd.vTranslate.x - vTranslateXEnd), vFocus.y
						+ (satEnd.vTranslate.y - vTranslateYEnd));
			}
	...
	public static interface OnImageEventListener {
		 ...
		 * either a preview image, the full size image, or base layer tiles are
		 * loaded. This indicates the scale and translate are known and the next
		 * draw will display an image. This event can be used to hide a loading
		 ...
		 * means the lowest resolution base layer of tiles are loaded, and when
		 * tiling is disabled, the image bitmap is loaded. This event could be
		 * used as a trigger to enable gestures if you wanted interaction
		 * disabled while only a preview is displayed, otherwise for most cases
		 * {@link #onReady()} is the best event to listen to.
		 */
		void onImageLoaded();

		/**
		 * Called when a preview image could not be loaded. This method cannot
		 ...
		 * detectable error. The view will continue to load the full size image.
		 * 
		 * @param e
		...
		/**
		 * Indicates an error initiliasing the decoder when using a tiling, or
		 * when loading the full size bitmap when tiling is disabled. This
		 * method cannot be relied upon; certain encoding types of supported
		 * image formats can result in corrupt or blank images being loaded and
		 * displayed with no detectable error.
		 * 
		 * @param e
		 *            The exception thrown. This error is also logged by the
		 *            view.
		 */
		 ...
		 * relied upon; certain encoding types of supported image formats can
		 * result in corrupt or blank images being loaded and displayed with no
		 * detectable error. Most cases where an unsupported file is used will
		 * result in an error caught by {@link #(Exception)}.
		 * 
		 * @param e
		 *            The exception thrown. This error is logged by the view.
		 */
