...
public class XListViewFooter extends LinearLayout {
	...
	public void setFootText(int code){
        initViewFooter();
		setHintTextView(code);
    }

    public void setHintTextView(int code){
        if(code == -2){
            ToastManager.getInstance().showToast(getContext(), "网络无效，请检测网络");
        }else if(code == 200 || code == 201){
            ToastManager.getInstance().showToast(getContext(), "刷新成功");
        }else{
            ToastManager.getInstance().showToast(getContext(), "刷新失败");
        }
    }
