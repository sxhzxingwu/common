...
public class CBProgressBar extends View {
	 ...
	 * 进度提示文字大小
	 **/
	private int percenttextsize = 14;
	/**
	 * 进度提示文字颜色
	 **/
	...
	public CBProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		strokeWidth = (int) array.getDimension(R.styleable.cbprogressbar_cbp_stroke_w, strokeWidth);
	...
	private void drawRoundProgressBar(Canvas canvas, Paint piant) {
		...
		canvas.drawArc(oval, -90, 360f * progress / max, false, piant);
		// 画进度文字
	...
	private void drawHoriRectProgressBar(Canvas canvas, Paint piant) {
		...
		// 画水平矩形
		canvas.drawRoundRect(new RectF(centerX - getWidth() / 2, centerY - getHeight() / 2, centerX + getWidth() / 2,
				centerY + getHeight() / 2), rectRound, rectRound, piant);
		...
		if (isHorizonStroke) {
			canvas.drawRoundRect(
					new RectF(centerX - getWidth() / 2, centerY - getHeight() / 2,
							((progress * 100 / max) * getWidth()) / 100, centerY + getHeight() / 2),
					rectRound, rectRound, piant);
		} else {
...
//		float textHeight = rect.height();
	...
	private String getPercent(){
		float percent = (progress * 100f / max);
		if(percent>1) return String.valueOf((int)percent);
		return String.format(new Locale(""), "%.2f", percent);
	}
	/**
	 * 绘制扇形扫描式进度
	 * 
	...
	private void drawSectorProgressBar(Canvas canvas, Paint piant) {
		...
		canvas.drawArc(oval, -90f, 360f * progress / max, true, piant);
	}
