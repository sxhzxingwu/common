...
package com.android.weici.common.widget.wheel;
...
import com.android.weici.common.widget.wheel.adapter.WheelViewAdapter;
...
public class WheelView extends View {
	...
	private void initData(AttributeSet attrs) {
		...
		if(attrs != null){
            ...
            padding = (int)a.getDimension(R.styleable.WheelView_item_padding, padding);
            isCyclic = a.getBoolean(R.styleable.WheelView_cyclic, isCyclic);
            currentItem = a.getInteger(R.styleable.WheelView_current_item, currentItem);
            visibleItems = a.getInteger(R.styleable.WheelView_visible_items, visibleItems);
            a.recycle();
        }
	}
	 ...
	 */
	private void setVisibleItems(int count) {
		visibleItems = count;
	}
	 ...
	 * @param index
	 *            the item index
	 * @param auto false手动触发，true 滑动触发
	 *            the animation flag
	 */
	private void setCurrentItem(int index, boolean auto) {
		...
		if (index != currentItem) {
			/*if (animated) {
				...
				if(auto)
				    notifyChangingListeners(old, currentItem);//手动设置currentItem不需要触发listener
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		width = Tools.dip2px(getContext(), 80);
	...
	private void drawShadows(Canvas canvas) {
		int height = (int) (1.5 * getItemHeight()) + 1;
	...
	private void drawItems(Canvas canvas) {
		...
		canvas.translate(padding, scrollingOffset - top);
	...
	private void createItemsLayout() {
		if (itemsLayout == null) {
			...
			itemsLayout.setGravity(Gravity.END);
		}
	}
